...
Strands multi-agent 能力分阶段进入平台：agents-as-tools 可以进入普通 platform session；Graph 和 Swarm 先作为实验 API 进入，再在事件模型稳定后 session 化。
...
M5 先引入 Graph 和 Swarm 实验 API。M7 将这些 run session 化：每次 run 创建 platform session，把 `multi_agent.*` events 写入 append-only event log，并可通过现有 session events API 和 Console timeline 查看。M8 增加可复用 workflow definitions 和 visual builder，同时仍把每次 run 映射回 session truth。Node agents 仍只使用各自的 model、provider、system prompt 和 skills，不加载 tools/MCP/agentTools，以降低 trajectory contract 成熟前的副作用。
...
- 在 `Persistence` 后增加 SQLite persistence
- 在 `SecretStore` 后增加本地 provider API key 存储
- 增加 `GET /v1/platform/status`
- 增加 `Sandbox` 和 `LocalWorkspaceSandbox`
- 服务重启后恢复 sessions 和 event timelines
- Graph/Swarm 可视化 builder 继续延后

### M7：Multi-Agent Runs Session 化

- 增加 `SessionKind`，支持 `agent`、`graph`、`swarm`
- 把 Graph/Swarm runs 持久化为 platform sessions
- 增加 `multi_agent.run_started`、`multi_agent.node_result`、`multi_agent.run_completed`、`multi_agent.run_failed`
- 从 SQLite event log 恢复 multi-agent timelines
- Console 增加 `Runs` tab，提供简单 Graph/Swarm 表单
- Graph/Swarm visual builder 和可复用 workflow persistence 延后到 M8+

### M8：Multi-Agent 可视化 Builder

- 增加 `WorkflowDefinition` 和 `WorkflowStore`
- 增加 `/v1/workflows` CRUD 和 `/v1/workflows/:id/runs`
- 保存可复用 Graph/Swarm workflow definitions
- Console 增加可拖拽节点和 Graph 连线的 visual workflow builder
- 把 workflow runs 映射回 sessions 和 timeline events
- node-level live streaming、templates 和 import/export 延后到 M9+
